...
function observeFadeUps() {
  ...
  els.forEach((el) => {
    el.classList.remove("visible"); // reset khi đổi page
    obs.observe(el);
  });
}
...
// ✅ Đọc page từ pathname khi load/refresh
...
function initShopPage() {
  ...
  function filterShop(el, cat) {
    ...
    renderShopProducts(
      ...
      cat === 'all'
        ? products
        : products.filter(p => p.cat.toLowerCase() === cat.toLowerCase())
    );
  }
  window.filterShop = filterShop;

  // Render helper dùng riêng cho shop (tránh đè renderProducts global)
      ...
      return `
        <div class="product-card">
          ...
          <div class="product-info">
            ...
            <div class="stars" style="color:var(--gold);font-size:12px;margin-bottom:6px;">
              ★★★★★
              <span style="color:#887278;font-size:11px;">(${p.reviews || 0})</span>
            </div>
            <div class="price-row">
...
function initPawfeastPage() {
  ...
  $('s3-prev')?.addEventListener('click', () => {
  ...
  // resize: dùng local listener, tự cleanup khi rời page
...
function initMemberPage() {
  ...
  function switchPetTab(tab, btn) {
    ...
    // Khi đổi tab, reset về stage đầu tiên
    const stageMap = { dog: 'puppy', cat: 'kitten' };
    const firstStage = stageMap[tab];
    const firstEl = document.querySelector(`#${tab}-stages .stage-item`);
    if (firstEl) switchStage(tab, firstStage, firstEl);
  }
  window.switchPetTab = switchPetTab;

  /* ── STAGE SWITCHING ── */
  function switchStage(tab, stage, el) {
    // Update .stage-item active — tìm đúng container theo tab
    ...
    // Hide tất cả review-area của tab này
    const stageMap = {
      dog: ['puppy', 'canine', 'senior'],
    ...
    // Show panel được chọn + re-trigger animation
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  await loadComponent("chat-container", "components/chat.html");
  await loadComponent("auth-container",    "components/auth.html");
  // Trong DOMContentLoaded, sau khi load cart component:
  await loadComponent("cart-container", "components/cart.html");
  Cart.render(); // ✅ sync lại UI sau khi DOM đã có
  ...
  // ---- Hiện popup sau 1.5 giây (chỉ 1 lần mỗi session) ----
...
// ✅ popstate khi nhấn Back/Forward trên browser
window.addEventListener("popstate", (e) => {
...
function claimOffer() {
  ...
  // TODO: gửi email/name lên server của bạn ở đây
  // fetch('/api/promo', { method: 'POST', body: JSON.stringify({ name, email }) });
...
// ===============================
// AUTH MODAL (global — mọi page dùng được)
// ===============================
...
var Cart = (function () {
  ...
  function render() {
    ...
    body.innerHTML = items.map(function (item) {
      ...
      return '<div class="cd-item" id="cd-item-' + item.id + '">'
        + imgEl
        ...
        +     '<button class="cd-qty-btn" onclick="Cart.setQty(\'' + item.id + '\',' + (item.qty + 1) + ')">+</button>'
        +   '</div>'
        +   '<button class="cd-remove" onclick="Cart.remove(\'' + item.id + '\')">Remove</button>'
        + '</div>'
  ...
  return { add: add, remove: remove, setQty: setQty, count: count, total: total, render: render };
})();
...
function renderProducts(containerId, items) {
  ...
  c.innerHTML = items.map(function (p, idx) {
    ...
    return '<div class="product-card">'
      ...
      +   '<div class="product-name">' + p.name + '</div>'
      +   '<div class="stars" style="color:var(--gold,#c8a84b);font-size:12px;margin-bottom:8px;">★★★★★ '
      +     '<span style="color:#887278;font-size:11px;">(' + (p.reviews || 0) + ')</span>'
      +   '</div>'
      +   '<div class="price-row">'
      +     '<span class="price-new">' + p.price + '</span>'
...
function loaderStart() {
  ...
  loaderTimeouts.push(setTimeout(() => {
    el.style.width = '45%';
  }, 200));
...
function loaderDone() {
  ...
  // QUAN TRỌNG: không nhảy 100% ngay
  el.style.width = '100%';
